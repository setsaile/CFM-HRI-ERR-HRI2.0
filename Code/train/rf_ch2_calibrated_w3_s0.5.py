...
from sklearn.calibration import CalibratedClassifierCV  # 添加这一行导入语句
...
model_path = "./BASE/models/clf_ch2_calibrated_rf_win3_stri0.5.pkl"

if os.path.exists(model_path):
    print("Loading model from disk...")
    clf_reaction = joblib.load(model_path)
else:
    print(" • Training challenge 2 model with MiniRocket and Calibrated RandomForest…")
    
    # 使用更多内核以提高特征提取能力
    minirocket = MiniRocketWithProgress(
        num_kernels=2000,  # 增加内核数量以提高特征表达能力
        random_state=42
    )
    
    # 使用RandomForestClassifier作为基础分类器
    ...
    # 使用CalibratedClassifierCV包装RandomForest，以获得更准确的概率估计
    # 这对于设置阈值以平衡TP和FP非常重要
    ...
    print(" done (MiniRocket + Calibrated RandomForest model trained and saved).")
...
# 使用更高的阈值以减少FP，同时保持较高的TP
threshold = 0.2  # 提高阈值以减少FP
...
pred_test_df.to_csv("./BASE/result/y_pred_ch2_w3_s0.5_calibrated.csv", index = False)
