...
train_data_path = "./BASE/Data/rf_train_window_size_1_stride_1.csv"
val_data_path = "./BASE/Data/rf_val_window_size_1_stride_1.csv"
test_data_path = "./BASE/Data/rf_test_window_size_1_stride_1.csv"
...
model_path = "./BASE/models/clf_ch2_randomforest_w1_s1_new.pkl"

if os.path.exists(model_path):
    print("Loading model from disk...")
    clf_reaction = joblib.load(model_path)
else:
    print(" • Training challenge 2 model with RandomForest…")
    
    # instantiate 
    clf_reaction = RandomForestClassifier(class_weight='balanced', random_state=42)
    
    # 训练模型
    print("   Training classifier...")
    clf_reaction.fit(X_train_sm, y_train_sm)
...
# # 特征的重要性
# feat_imp = pd.Series(
#     clf_reaction.feature_importances_, 
...
# predict
# 第一个是验证集的,第二个是测试集的;验证集进行指标评判,测试集查看最终保存的格式
print("模型类别：", type(clf_reaction))

# 不需要reshape为3D格式，直接使用2D数据
...
# # 若模型存在则load,不存在重新训练
# if os.path.exists(model_path):
#     print("  • Loading existing model from file…", end="", flush=True)
#     # # 需要将模型结构进行初始化
#     # clf_reaction = make_pipeline(
#     #     MiniRocket(),             # 特征提取
#     #     RidgeClassifierCV()       # 分类器
#     # )
#     clf_reaction = joblib.load(model_path)
#     print("第一处clf类别：", type(clf_reaction))
#     print(" done (model loaded).")
# else:
#     print(" • Training challenge 2 model with MiniRocket…", end="", flush=True)
    
#     # 创建MiniRocket + RidgeClassifier管道
#     # MiniRocket用于特征提取，RidgeClassifier用于分类
#     minirocket = MiniRocket(
#         num_kernels=10000,  # 核函数数量，可调节
#         random_state=42
#     )
...
#     # 训练模型
#     # 注意：X_train_sm需要是3D数组格式 (n_samples, n_timepoints, n_features)
#     # 如果您的数据是2D的，可能需要reshape
#     clf_reaction.fit(X_train_sm, y_train_sm)
...
pred_test_df.to_csv("./BASE/result/y_pred_ch2_w1_s1_randomforest_new.csv", index = False)
