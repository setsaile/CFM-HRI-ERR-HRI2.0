...
# 设置模型保存路径
original_model_path = "./BASE/models/clf_ch2_newminirocket_win3_stri1.pkl"
filtered_model_path = "./BASE/models/clf_ch2_newminirocket_win3_stri1_filtered.pkl"
...
# 如果过滤后的模型不存在，则训练新模型
    ...
    print("   Step 4: Creating pipeline and saving...")
    clf_reaction = make_pipeline(minirocket, classifier)
    joblib.dump(clf_reaction, original_model_path)
    joblib.dump(clf_reaction, filtered_model_path)  # 同时保存为过滤后的模型
...
# 预测验证集
print("在验证集上进行预测...")
probs_reaction = clf_reaction.predict_proba(X_val_reshaped)[:, 1]

# 使用较低的阈值以提高TP，同时尽量控制FP
threshold = 0.35  # 调整阈值以平衡TP和FP
print(f'threshold: {threshold}')
pred_reaction = (probs_reaction >= threshold).astype(int)

# 预测测试集
print("在测试集上进行预测...")
probs_reaction_test = clf_reaction.predict_proba(X_test_reshaped)[:, 1]
...
pred_test_df.to_csv("./BASE/result/y_pred_ch2_w3_s1_newminirocket_filter.csv", index=False)
...
# 输出模型性能
print("\nMiniRocket模型性能:")
print(f"True Positive Rate: {tp/total_reaction*100:.2f}%")
print(f"False Positive: {fp}")
