...
# 释放不再需要的数据
del X_train_imputed, y_err_train
gc.collect()
...
# 修改为
X_train_balanced_df = pd.DataFrame(X_train_balanced, columns=df_train.drop(columns=[
    'start','end','robot_error','reaction_ch1','reaction_ch2',
    'reaction_type','system','task','trial'
]).columns)
...
# 设置模型保存路径
ensemble_model_path = "./BASE/models/clf_ch1_ensemble_win5_stri0.5.pkl"
...
if os.path.exists(ensemble_model_path):
    ...
    # 1. MiniRocket模型
    print("   训练MiniRocket模型...")
    minirocket_model_path = "./BASE/models/clf_ch1_minirocket_win5_stri0.5_ensemble.pkl"
    
    if os.path.exists(minirocket_model_path):
        print("   从磁盘加载MiniRocket模型...")
        ...
        # 使用较少的内核数量以减少过拟合风险
        minirocket = MiniRocketWithProgress(num_kernels=1500, random_state=42)  # 减少内核数量
        ...
        minirocket_clf = LogisticRegressionCV(cv=3, random_state=42, class_weight='balanced', max_iter=1000)  # 减少CV折数
    ...
    rf_model_path = "./BASE/models/clf_ch1_rf_win5_stri0.5_ensemble.pkl"
    
    if os.path.exists(rf_model_path):
        print("   从磁盘加载RandomForest模型...")
    ...
    # 3. LightGBM模型
    print("   训练LightGBM模型...")
    lgb_model_path = "./BASE/models/clf_ch1_lgb_win5_stri0.5_ensemble.pkl"
    
    if os.path.exists(lgb_model_path):
        print("   从磁盘加载LightGBM模型...")
        ...
        with tqdm(desc="LightGBM training") as pbar:
            lgb_clf.fit(
                ...
                eval_set=[(X_val_imputed, y_err_val)],
                eval_metric='auc',
    ...
    gb_model_path = "./BASE/models/clf_ch1_gb_win5_stri0.5_ensemble.pkl"
    
    if os.path.exists(gb_model_path):
        print("   从磁盘加载GradientBoosting模型...")
    ...
    # 释放训练数据内存
    del X_train_balanced, X_train_balanced_df, y_train_balanced
    gc.collect()
    ...
    print(" 完成 (集成模型训练并保存完成).")
    
    # 交叉验证评估
    print("执行交叉验证...")
    cv_scores = cross_val_score(ensemble_clf, X_val_imputed, y_err_val, cv=3, scoring='f1')  # 减少CV折数
    print(f"交叉验证F1分数: {cv_scores}")
    print(f"平均CV F1分数: {cv_scores.mean():.4f} ± {cv_scores.std():.4f}")
...
pred_test_df.to_csv("./BASE/result/y_pred_ch1_w5_s0.5_ensemble.csv", index=False)
...
# 在 MiniRocketWithProgress 类之后，添加以下两个类定义
