...
model_path = "./BASE/models/clf_ch2_minirocket_win3_stri0.5.pkl"
...
if os.path.exists(model_path):
    ...
    print(" • Training challenge 2 model with MiniRocket…")
...
print("第二处clf类别：", type(clf_reaction))
X_val = X_val.to_numpy().reshape((X_val.shape[0], X_val.shape[1], 1))
X_val = np.transpose(X_val, (0, 2, 1))
probs_reaction = clf_reaction.predict_proba(X_val)[:, 1]
threshold = 0.3
...
probs_reaction_test = clf_reaction.predict_proba(X_test)[:, 1]
threshold = 0.3
...
pred_test_df.to_csv("./BASE/result/y_pred_ch2_w3_s0.5_minirocket.csv", index = False)
