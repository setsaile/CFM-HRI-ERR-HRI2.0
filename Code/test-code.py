...
def evaluate_error(y_pred_df, y_true_df):
    ...
    pos_pred = y_pred_df.loc[y_pred_df['y_pred_err'] == 1].copy()
    ...
    if len(y_true_df) > 0:
        for _, row in y_true_df.iterrows():
            ...
            error_onset=row['start'].total_seconds() - 1 # added one second tolerance
            error_offset=row['end'].total_seconds() + 1 # added one second tolerance
...
# print(" done.")
model_path = "./BASE/rf_challenge1_model.pkl"
